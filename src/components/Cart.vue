    ...
    <div class="cart-content">
        <div class="cart-box1" v-if="shoplist.length>0">
            ...
            <div class="account" v-if="account">
                ...
                <span class="accountall" @click="save">结算({{Totalcartnum}})</span>
            </div>
...
export default {
    ...
    methods:{
        ...
        save(){
            ...
            //获取购物车的数据
            let cart = this.$store.state.shoplist
            if(cart.length>0){

                this.axios.get('/api/save',{params:{cart:JSON.stringify(cart)}}).then((res)=>{
